  ...
  module Oss
    module Api
      module BucketProperty
        ...
        #
        # @param target_bucket [String] specifies the bucket where you want ZAliyun OSS to store server access logs.
        ...
        #
        # @param rules [Array<ZAliyun::Oss::Struct::LifeCycle>] rules for lifecycle
        #
        # @raise [RequestError]
        # @raise [ZAliyun::Oss::InvalidLifeCycleRuleError]
        #   if rule invalid
        #
        # @return [Response]
        def bucket_enable_lifecycle(rules = [])
          ...
          rules.each do |rule|
            unless rule.valid?
              fail ZAliyun::Oss::InvalidLifeCycleRuleError, rule.inspect
            end
          end
        ...
        #
        # @param rules [Array<ZAliyun::Oss::Struct::Cors>] array of rule
        #
        # @raise [RequestError]
        ...
        def bucket_enable_cors(rules = [])
          ...
          rules.each do |rule|
            unless rule.valid?
              fail ZAliyun::Oss::InvalidCorsRuleError, rule.inspect
            end
          end
